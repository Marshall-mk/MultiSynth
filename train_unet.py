...
class UNetLoss(nn.Module):
    ...
    def __init__(
        ...
        # Initialize perceptual if requested
        if use_perceptual:
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        # Compute perceptual if enabled
        if self.use_perceptual and self.perceptual_loss is not None:
            perceptual_val = self.perceptual_loss(pred, target)
            losses['perceptual'] = perceptual_val
            total_loss = total_loss + self.perceptual_weight * perceptual_val
        else:
...
def validate(
    ...
    with torch.no_grad():
        for batch_data in tqdm(
            ...
            val_perceptual_losses.append(loss_dict['perceptual'].item())
...
def train_unet_model(
    ...
    Args:
        ...
        use_stacks: Stack selection ("all", "012", "01", "02", "12", "0", "1", "2")
        ...
        perceptual_weight: Weight for LPIPS loss
        use_perceptual: Enable LPIPS perceptual loss
        perceptual_network: Backend for 3D-LPIPS ('medicalnet', 'monai', 'models_genesis')
    ...
    if accelerator.is_main_process:
        ...
        if use_perceptual:
            loss_info += f", perceptual={perceptual_weight} (backend={perceptual_network})"
        print(loss_info)
    ...
    # Move criterion to device (needed for submodules like PerceptualLoss/LPIPS)
    ...
    if accelerator.is_main_process and csv_log:
        ...
        csv_headers = ["epoch", "step", "train_loss", "train_l1", "train_ssim", "train_perceptual",
                       "val_loss", "val_l1", "val_ssim", "val_perceptual", "val_mae", "val_psnr", "lr", "epoch_time"]
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_idx, batch_data in enumerate(pbar):
            ...
            epoch_perceptual_losses.append(loss_dict['perceptual'].item())
            ...
            # Add perceptual to progress bar if enabled
            if use_perceptual:
                postfix_dict["perceptual"] = f"{loss_dict['perceptual'].item():.4f}"
            ...
            if use_wandb and accelerator.is_main_process and global_step % 10 == 0:
                ...
                if use_perceptual:
                    wandb_log['train/perceptual'] = loss_dict['perceptual'].item()
                wandb.log(wandb_log, step=global_step)

        avg_loss = np.mean(epoch_losses)
        avg_l1 = np.mean(epoch_l1_losses)
        avg_ssim = np.mean(epoch_ssim_losses)
        avg_perceptual = np.mean(epoch_perceptual_losses) if use_perceptual else 0.0
        ...
        if val_dataloader and (epoch + 1) % val_interval == 0:
            ...
            if use_perceptual:
                train_info += f", Perceptual: {avg_perceptual:.4f}"
            train_info += ")"
    ...
    if accelerator.is_main_process:
        ...
        if use_wandb:
            artifact = wandb.Artifact(
                name=f"{model_type}-{wandb.run.id}",
                type="model",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_stacks", type=str, default="all",
                        help="Which stacks to use: 'all'/'012' (3 stacks), '01'/'02'/'12' (2 stacks), '0'/'1'/'2' (1 stack)")
    ...
    # Model selection
    parser.add_argument("--model_type", type=str, default="segresnet",
                        choices=["segresnet", "swinunetr"],
                        help="Model architecture to train (default: segresnet)")
    ...
    parser.add_argument("--use_perceptual", action="store_true", help="Enable perceptual loss")
    parser.add_argument("--perceptual_network", type=str, default="alex",
                        choices=["alex", "vgg", "radimagenet"],
                        help="Backend for perceptual loss")
