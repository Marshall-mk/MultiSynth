...
def run_pipeline(
    ...
    if do_register:
        if verbose:
            print(f"=== Registering to image {fixed_idx} ===")
        ...
        if save_registered:
            if verbose:
                print("=== Saving registered outputs ===")
...
Examples:
  ...
  # Only register (assumes already resampled)
  python pipeline.py -i a.nii.gz b.nii.gz -o out/ --no-resample --no-crop

  # Resample + crop (skip registration)
  python pipeline.py -i a.nii.gz b.nii.gz -o out/ --no-register

  # Save both registered and cropped outputs
  python pipeline.py -i a.nii.gz b.nii.gz -o out/ --save-registered
  ...
  python pipeline.py -i a.nii.gz b.nii.gz -o out/ -r 0.5 0.5 0.5 --metric meansquares --transform Affine
    ...
    parser.add_argument("-i", "--inputs", nargs="+", required=True, help="Input NIfTI files")
    ...
    parser.add_argument("--interp", choices=["trilinear", "bilinear", "nearest"], default="trilinear")
    ...
    parser.add_argument("--metric", choices=["mattes", "meansquares", "gc"], default="gc", help="Registration metric (default: mattes)")
